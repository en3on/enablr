@names = [
  "Morus Zero | Ultra-fast countertop tumble dryer for any home",
  "Narwal: World's First Self-Cleaning Robot Mop & Vacuum",
  "Pocket Sky. Activate yourself with light. Anytime, anywhere.",
  "IVI: The Closed-Loop 3D Printer",
	"Snapmaker 2.0: Modular 3-in-1 3D Printers",
	"The Orijin Sponge | Enhance Your Daily Routine",
	"Demine Robotics: Saving Land, Limbs and Lives",
  "Voltage Research Laboratory Organic Modular Synthesizer",
	"TRONEX - A New Generation Of Flashlights",
	"Backmate | The Easiest Way to Relieve Back Pain",
	"Chimera 5-in-1 Cable | The World's Most Versatile Cable",
	"Chatterbox: the smart speaker that kids build and program"
]
